...
public class EpisodeListAsyncTask extends AsyncTask<Void, Void, List<Episode>> {
    ...
    protected List<Episode> doInBackground( Void... params ) {
        return mKatgService.listEpisodes( mShowNameId, mShowId, mNumber, mLimit );
    }
    ...
    protected void onPostExecute( List<Episode> episodes ) {
        ...
        for( Episode episode : episodes ) {
            ...
            if( cursor.moveToFirst() ) {
                ...
                mContext.getContentResolver().update(ContentUris.withAppendedId(EpisodeConstants.CONTENT_URI, id), values, null, null);
                ...
                mContext.getContentResolver().insert( EpisodeConstants.CONTENT_URI, values );
            ...
            if( null != episode.getGuests() && episode.getGuests().length > 0 ) {
                ...
                for( Guest guest : episode.getGuests() ) {
                    ...
                    if( cursor.moveToFirst() ) {
                        ...
                        mContext.getContentResolver().update(ContentUris.withAppendedId(GuestConstants.CONTENT_URI, id), values, null, null);

                    } else {
                        Log.v( TAG, "onPostExecute : guest iteration, adding new entry" );

                        mContext.getContentResolver().insert(GuestConstants.CONTENT_URI, values);
                    ...
                    if( cursor.moveToFirst() ) {
                        ...
                        mContext.getContentResolver().update(ContentUris.withAppendedId(EpisodeGuestConstants.CONTENT_URI, id), values, null, null);

                    } else {
                        Log.v( TAG, "onPostExecute : episodeGuest iteration, adding new entry" );

                        mContext.getContentResolver().insert(EpisodeGuestConstants.CONTENT_URI, values);
                ...
                if( !guestNames.isEmpty() ) {
                    ...
                    mContext.getContentResolver().update(ContentUris.withAppendedId(EpisodeConstants.CONTENT_URI, episode.getShowId()), values, null, null);
